...
#ifdef _DEBUG
    #define USE_DEBUG(...)              DbgPrint( __VA_ARGS__); printf( __VA_ARGS__);
#endif
...
#endif
    int64_t start_offset; // Total number of bytes sent
    int torrentId;
};
                                     ...
                                     struct cgi_env_block *blk )
{
    ...
    if ( (s = mg_get_header( ri, "Content-Type" )) != NULL ) addenv( blk, "CONTENT_TYPE=%s", s );

    if ( ri->query_string != NULL ) addenv( blk, "QUERY_STRING=%s", ri->query_string );

    if ( (s = mg_get_header( ri, "Content-Length" )) != NULL ) addenv( blk, "CONTENT_LENGTH=%s", s );
...
const char* mg_get_header( const struct mg_connection *ri, const char *s )
{
    ...
    for ( i = 0; i < ri->num_headers; i++ ) if ( !mg_strcasecmp( s, ri->http_headers[i].name ) ) return ri->http_headers[i].value;
...
static int get_month_index( const char *s )
{
    ...
    for ( i = 0; i < (int) ARRAY_SIZE( month_names ); i++ ) if ( !strcmp( s, month_names[i] ) ) return i;
                            ...
                            size_t path_len, file_stat_t *stp )
{
    ...
    while ( (list = next_option( list, &filename_vec, NULL )) != NULL )
    {
        ...
        if ( filename_vec.len > (int) (path_len - (n + 2)) ) continue;
...
extern int getTorrentIndex( const char *uri );
extern uint64_t getFileSize( int torrentId );
...
//  conn->cl = st->st_size;
    conn->torrentId = getTorrentIndex( conn->mg_conn.uri );
    if ( conn->torrentId < 0 ) conn->torrentId = getTorrentIndex( conn->mg_conn.uri );
    totalLen = getFileSize( conn->torrentId );
    conn->cl = totalLen;
    range[0] = '\0';
    ...
    USE_DEBUG( "Range hdr %s\n", hdr );
    USE_DEBUG( "Get url %s\n", conn->mg_conn.uri );
    if ( hdr != NULL && (n = parse_range_header( hdr, &r1, &r2 )) > 0 &&
         r1 >= 0 && r2 >= 0 )
    {
//      extern uint64_t getFileSize( const char *uri );
//      uint64_t knownSize = getFileSize( conn->mg_conn.uri );
        ...
        mg_snprintf( range, sizeof(range), "Content-Range: bytes "
                     ...
                     r1, r1 + conn->cl - 1, (int64_t) totalLen );
        USE_DEBUG( "Range %I64d-%I64d: %s ( \n", r1, r2, range );
        conn->start_offset = r1;
        msg = "Partial Content";
        lseek( conn->endpoint.fd, r1, SEEK_SET );
    }
//  else USE_DEBUG( "Range error\n" );
...
int mg_authorize_digest( struct mg_connection *c, FILE *fp )
{
    ...
    while ( fgets( line, sizeof(line), fp ) != NULL )
    {
        if ( sscanf( line, "%[^:]:%[^:]:%s", f_user, f_domain, ha1 ) == 3 &&
             ...
             !strcmp( conn->server->config_options[AUTH_DOMAIN], f_domain ) ) return check_password( c->request_method, ha1, uri,
...
extern uint64_t getFileDownloaded( int torrentId );

static void transfer_file_data( struct connection *conn )
{
    char *buf;
    int n;
    int64_t downloaded, size;
//  char buf[IOBUF_SIZE];
    if ( (buf = (unsigned char *) malloc( IOBUF_SIZE_GET )) == NULL ) return;

    if ( conn->torrentId < 0 ) conn->torrentId = getTorrentIndex( conn->mg_conn.uri );
    downloaded = getFileDownloaded( conn->torrentId );
    size = getFileSize( conn->torrentId );
    if ( downloaded != size && ( conn->start_offset < size - IOBUF_SIZE_GET ) )
    {
        if ( downloaded < conn->start_offset + conn->num_bytes_sent + IOBUF_SIZE_GET )
        {
            USE_DEBUG( "Stalled. offset %I64d, sent %I64d, downloaded %I64d/%I64d \n",
                       conn->start_offset, conn->num_bytes_sent, downloaded, size );
            Sleep( 1000 );
            return;
        }
    }
    n = read( conn->endpoint.fd, buf, conn->cl < (int64_t) IOBUF_SIZE_GET ?
              (int) conn->cl : (int) IOBUF_SIZE_GET );
    USE_DEBUG( "Reading %i sent %I64d left %I64d\n", n, size - conn->start_offset - conn->cl, conn->cl );
